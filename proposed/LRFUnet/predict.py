...
def predict_cases_fastest(model, list_of_lists, output_filenames, folds, num_threads_preprocessing,
                          ...
                          finetune=None, LoRA_r=4, factorization=None, low_rank_conv=None, LRF_r=16, Tucker_threshold=0.95, start_pos=0, use_SE=False, use_CBAM=False, use_ASPP=False):
    assert len(list_of_lists) == len(output_filenames)
    if segs_from_prev_stage is not None: assert len(segs_from_prev_stage) == len(output_filenames)
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    if mode == "normal":
        ...
        return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                             ...
                             segmentation_export_kwargs=segmentation_export_kwargs, finetune=finetune, LoRA_r=LoRA_r, factorization=factorization, low_rank_conv=low_rank_conv, LRF_r=LRF_r, Tucker_threshold=Tucker_threshold, start_pos=start_pos, use_SE=use_SE, use_CBAM=use_CBAM, use_ASPP=use_ASPP)
    elif mode == "fast":
        if overwrite_all_in_gpu is None:
            all_in_gpu = True
        else:
        ...
        return predict_cases_fastest(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                                     num_threads_preprocessing, num_threads_nifti_save, lowres_segmentations,
                                     tta, mixed_precision=mixed_precision, overwrite_existing=overwrite_existing, all_in_gpu=all_in_gpu,
                                     step_size=step_size, checkpoint_name=checkpoint_name, finetune=finetune, LoRA_r=LoRA_r, factorization=factorization, low_rank_conv=low_rank_conv, LRF_r=LRF_r, Tucker_threshold=Tucker_threshold, start_pos=start_pos, use_SE=use_SE, use_CBAM=use_CBAM, use_ASPP=use_ASPP)
    else:
