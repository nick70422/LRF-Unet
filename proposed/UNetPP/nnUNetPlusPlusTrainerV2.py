        ...
        if self.factorization == 'LRF':
            if self.low_rank_conv == 'AB':
                ...
                self.network = Generic_UNetPlusPlus_LRF(self.num_input_channels, self.base_num_features, self.num_classes,
                                            ...
                                            net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
                                            self.net_num_pool_op_kernel_sizes, self.net_conv_kernel_sizes, False, True, True,
                                            LRF_type=self.low_rank_conv, LRF_r=self.LRF_r, LRF_start_pos=self.start_pos, use_SE=self.use_SE, use_CBAM=self.use_CBAM, use_ASPP=self.use_ASPP, use_attn=self.use_attn)
...
if __name__ == '__main__':
    ...
    trainer = nnUNetPlusPlusTrainerV2(plans_file=plans_file, fold=0, output_folder="", dataset_directory="", batch_dice=True, stage=0, factorization='Tucker', Tucker_threshold=0.9)
