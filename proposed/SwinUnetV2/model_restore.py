...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None, use_LoRA=False):
    """
...
#    tr = recursive_find_python_class([search_in], name, current_module="nnunet.training.network_training")
    ...
    trainer = tr(*init, test=True, use_LoRA=use_LoRA)
...
def load_model_and_checkpoint_files(folder, folds=None, mixed_precision=None, checkpoint_name="model_best", use_LoRA=False):
    """
    ...
    trainer = restore_model(join(folds[0], "%s.model.pkl" % checkpoint_name), fp16=mixed_precision, use_LoRA=use_LoRA)
