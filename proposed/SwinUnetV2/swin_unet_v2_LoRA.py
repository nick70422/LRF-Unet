...
class LoRA(nn.Module):
    def __init__(self, dim, r, lora_alpha=1., lora_dropout=0.1):
        super().__init__()
        self.r = r
        ...
        nn.init.normal_(self.lora_A, mean=0, std=1 / math.sqrt(r))  
        nn.init.zeros_(self.lora_B)

    def forward(self, x):
        x_dropout = self.lora_dropout(x)
        x_A = x_dropout @ self.lora_A.T
        x_BA = x_A @ self.lora_B.T
        return x_BA * self.scaling
    
class WindowAttention_LoRA(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., LoRA_location='QKVO'):
        ...
        # 添加LoRA模块
        if 'Q' in self.LoRA_location:
            self.q_lora = LoRA(dim, r=4)  # r是LoRA的秩,可以根据需要调整
        if 'K' in self.LoRA_location:
            self.k_lora = LoRA(dim, r=4)
        if 'V' in self.LoRA_location:
            self.v_lora = LoRA(dim, r=4)
        if 'O' in self.LoRA_location:
            self.o_lora = LoRA(dim, r=4)
    ...
    def forward(self, x, mask=None):
        ...
        # 应用LoRA
        if 'Q' in self.LoRA_location:
            q = q + self.q_lora(q.permute(0, 2, 1, 3).reshape(B_, N, -1)).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        if 'K' in self.LoRA_location:
            k = k + self.k_lora(k.permute(0, 2, 1, 3).reshape(B_, N, -1)).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        if 'V' in self.LoRA_location:
            v = v + self.v_lora(v.permute(0, 2, 1, 3).reshape(B_, N, -1)).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 use_LoRA=False, LoRA_location='QKVO'):
        super().__init__()
        self.dim = dim
        ...
        if use_LoRA:
            self.attn = WindowAttention_LoRA(
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 norm_layer=norm_layer, use_LoRA=use_LoRA, LoRA_location=LoRA_location)
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_LoRA=False, LoRA_location='QKVO', **kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               use_LoRA=use_LoRA, LoRA_location=LoRA_location)
